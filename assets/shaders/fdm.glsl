...
float light(vec2 st, float t, float w = 0.2) { 
...
float test4(vec2 st, float t)
{
    ...
    for(float i = -1.0; i < 2.0; i+= 0.1) { 
        count++;
        vec2 off = vec2(10.0,-10.3 + i + k * 6.0 * sin(iTime * 0.002)) * 10;
        vec2 p = off + st;
        float lig = light(p, t);
...
float plotFft(vec2 st, float t) { 
    ...
    float func = max(1 - abs(value - st.y),0);
    if(func < 0.98) return pow(func, 40);
    return func;
}
void main() { 
    vec2 st = getSt();
    float result = fft(st, iTime);
